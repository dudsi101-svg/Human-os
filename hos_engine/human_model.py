...
class HumanRecord:
 record_id:str; subject_id:str; domain:str; key:str; value:Any; evidence_type:EvidenceType; confidence:float; source_id:str; created_at:str; status:RecordStatus=RecordStatus.ACTIVE; supersedes:Optional[str]=None; sensitive:bool=False; tags:Set[str]=field(default_factory=set)
class HumanModel:
 ...
 def add(self,*,subject_id,domain,key,value,evidence_type,confidence,source_id,sensitive=False,tags=None,supersedes=None):
  ...
  r=HumanRecord('HOS-HMR-'+uuid.uuid4().hex[:12].upper(),subject_id,domain,key,value,evidence_type,confidence,source_id,datetime.now(timezone.utc).isoformat(),supersedes=supersedes,sensitive=sensitive,tags=set(tags or [])); self._records[r.record_id]=r
